...
def train(rank, model, world_size):
    ...
    train_loader = DataLoader(train_dset, shuffle=False, batch_size=64, sampler=train_sampler)
    test_loader = DataLoader(test_dset, shuffle=False, batch_size=64)
    ddp_model = DDP(model, device_ids=[rank])
    optimizer = optim.AdamW(ddp_model.parameters(), lr=1e-3)
    criterion = torch.nn.CrossEntropyLoss()

    def get_model_randomly():
        randn_number = torch.randint(0, 10, (1,))
        if randn_number.item() % 2 == 0:
            return model
        else:
            return ddp_model
    ...
    for epoch in range(5):
        ...
        for batch_idx, (data, target) in enumerate(train_loader):
            data, target = data.to(rank), target.to(rank)
            output = get_model_randomly()(data)
            target = target.long()
            loss = criterion(output, target)
            loss.backward()
            optimizer.step()
